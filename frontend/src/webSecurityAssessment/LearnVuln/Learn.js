...
//   const vulnerabilities = [
    // {
    //   title: 'SQL Injection',
    //   content: ['A SQL injection attack consists of insertion or “injection” of a SQL query via the input data from the client to the application. A successful SQL injection exploit can read sensitive data from the database, modify database data (Insert/Update/Delete), execute administration operations on the database (such as shutdown the DBMS), recover the content of a given file present on the DBMS file system and in some cases issue commands to the operating system. SQL injection attacks are a type of injection attack, in which SQL commands are injected into data-plane input in order to affect the execution of predefined SQL commands.', 'SQL injection attack occurs when:', '1. An unintended data enters a program from an untrusted source.', '2. The data is used to dynamically construct a SQL query', 'The main consequences are:', '\u2022 Confidentiality: Since SQL databases generally hold sensitive data, loss of confidentiality is a frequent problem with SQL Injection vulnerabilities.', '\u2022 Authentication: If poor SQL commands are used to check user names and passwords, it may be possible to connect to a system as another user with no previous knowledge of the password.', '\u2022 Authorization: If authorization information is held in a SQL database, it may be possible to change this information through the successful exploitation of a SQL Injection vulnerability.', '\u2022 Integrity: Just as it may be possible to read sensitive information, it is also possible to make changes or even delete this information with a SQL Injection attack.', 'Preventing SQL Injection', '\u2192 User Authentication: Validating input from the user by pre-defining length, type of input, of the input field and authenticating the user.', '\u2192 Restricting access privileges of users and defining how much amount of data any outsider can access from the database. Basically, users should not be granted permission to access everything in the database.', '\u2192 Use extensive data Sanitization: All user input must be filtered by websites. Ideally, user data should be context-filtered. Email addresses, for example, should be filtered to allow only the characters permitted in an e-mail address, phone numbers should be filtered to allow only the characters permitted in a phone number, and so on.', '\u2192 Because SQL injection vulnerabilities are frequently discovered in commercial software, it is critical to keep up with updating.'],
    //   references: ['https://shorturl.at/noy48', 'https://owasp.org/www-community/attacks/SQL_Injection'],
    //   images: ['https://spanning.com/wp-content/uploads/2019/07/SQL-injection-attack-example.png','https://media.geeksforgeeks.org/wp-content/uploads/20230321182818/SQL-Injection.jpg',], // Add local image paths
    // },
    // {
    //   title: 'Broken Authentication',
    //   content: ['Broken authentication is an umbrella term for several vulnerabilities that attackers exploit to impersonate legitimate users online. Broadly, broken authentication refers to weaknesses in two areas: session management and credential management. Both are classified as broken authentication because attackers can use either avenue to masquerade as a user: hijacked session IDs or stolen login credentials.','Attackers employ a wide variety of strategies to take advantage of these weaknesses, ranging from huge credential stuffing attacks to highly targeted schemes aimed at gaining access to a specific person’s credentials.', 'In recent years, broken authentication attacks have accounted for many of the worst data breaches, and security experts sound the alarm about this underrecognized threat. The Open Web Application Security Project (OWASP) has included it in its “Top 10” list of the biggest web application security risks since 2017. By 2020, broken authentication had climbed to the number two spot.', 'Follow the provided links to read about various types of attacks and exploits associated with Broken Authentication.'],
    //   references: ['https://auth0.com/blog/what-is-broken-authentication/', 'https://auth0.com/blog/what-is-broken-authentication/'],
    //   images: ['https://www.prplbx.com/wp-content/uploads/figure5-password-reset-poisoning.png',], // Add local image paths
    // },
    // {
    //   title: 'Sensitive Data Exposure',
    //   content: ['Sensitive data exposure refers to the accidental or deliberate disclosure of critical information such as personally identifiable information (PII), payment card information (PCI), electronic protected health information (ePHI), and intellectual property (IP). Organizations with inadequate data protection measures create vulnerabilities within the system, leading to sensitive data exposure.', 'Sensitive data exposure differs from a data breach, where malicious entities exploit vulnerabilities to carry out attacks intended to steal, misuse, or destroy sensitive personal data.', 'Causes of Sensitive Data Exposure: ', '\u2022 No encryption or weak encryption: Applications, NAS devices, database servers, and other repositories that are unencrypted or have weak encryption protocols are vulnerable to data exposure. In such cases, attackers can view vital information readily or crack the weak encryption in place easily.', '\u2022 Insecure passwords: When user credentials are stored as plain-text documents without being hashed and salted, they can be easily misused. Hashing and salting convert passwords to cipher texts that are difficult to decipher.', '\u2022 Unsecure webpages: Web applications and cloud storage with misconfigured SSL/HTTPS security protocols can lead to data being uploaded or downloaded without any encryption. Such unencrypted data in transit can be easily intercepted and exposed.', '\u2022 Poor access controls: Providing excessive permissions to users who don\'t need them and a lack of visibility into who has access to what files, empowers users to access and share data without any accountability.', '\u2022 Misconfiguration errors: Applications\' default permissions settings are meant for maximum usability, not security. When administrators fail to update the settings according to their data security requirements or miss security updates, it can lead to data exposure.'],
    ...
    //   images: ['https://assets-global.website-files.com/5ff66329429d880392f6cba2/613856435c93721a07ed5554_Sensitive%20Data%20Exposure%20Example.png', 'https://www.tutorialspoint.com/security_testing/images/sensitive_data_exposture.jpg'], // Add local image paths
    // },
    // {
    //   title: 'Broken Access Control',
    //   content: ['Access control, also known as authorization, is when a web application grants or denies users access to its content and features. It sounds like a simple step in the process of setting up a system, but establishing secure and correct access control requires resources.', 'Broken access control is a scenario in which an attacker breaks access control and can access, modify, delete or perform actions that the application would not initially allow. Many vulnerabilities can be treated as broken access control, for example, where an ordinary user can access admin-only functions by editing URLs or seeing or modifying the information of other users.', 'Cases of broken access control', '\u2022 Insecure IDs: Most URLs contain specific IDs, keys, or indexes to identify users, content units, or features. An attacker can guess and modify these IDs to gain access to information that should not be available to third parties. Implementing proper access control in the server is crucial so unauthorized persons cannot access the requested data.', '\u2022 Forced browsing past access control checks: Most websites have authorized access URLs that lie deeper down in the site and should only be accessible to authenticated users. Malicious actors usually find such endpoints using the directory brute-forcing tool through trial and error. This allows attackers to access admin-only pages or paid content that can only be viewed by those who have paid.', '\u2022 File path traversal: It is a website security vulnerability that allows a threat actor to access sensitive files on the application server. It might include operating system files, application code, or data credentials for backend system files. The breach can result in third parties modifying the sensitive data or taking complete control of the application.', '\u2022 Client side caching: Browsers cache website information to reach the endpoint more quickly if the user wants to reaccess the same site. To avoid such threats, developers can use HTML meta tags to prevent browsers from storing sensitive data in their cache.'],
    //   references: ['https://owasp.org/www-community/Broken_Access_Control','https://owasp.org/Top10/A01_2021-Broken_Access_Control/', ],
    //   images: ['https://prophaze.com/wp-content/uploads/2023/04/Broken-Access-Control.jpg','https://cheapsslsecurity.com/blog/wp-content/uploads/2022/06/broken-access-control.png',], // Add local image paths
    // },
    // {
    //   title: 'Security Misconfiguration',
    //   content: ['Security misconfiguration is any error or vulnerability present in the configuration of code that allows attackers access to sensitive data. There are many types of security misconfiguration, but most present the same danger: vulnerability to data breach and attackers gaining unauthorized access to data.', 'Security misconfiguration, because it involves flaws in security configuration, can lead to a data breach and even complete system compromise. Depending on the value of the data compromised, this can have a significant negative impact on a business. Attackers may be able to exploit or even modify parts of applications by taking advantage of security misconfigurations. These security misconfiguration vulnerabilities leave a business exposed to potential attack.', 'Examples of Security Misconfigurations: ', 'Some concrete examples of security misconfigurations include AD misconfigurations, vulnerabilities within the Active Directory domain. These common security misconfigurations range from attackers gaining administrative privileges to issues arising from services running on hosts with multiple administrators.', 'Another example is a security misconfiguration that was discovered in JIRA, a collaboration tool. One misconfiguration exposed many companies to the vulnerability of releasing corporate and personal data. In this case, it was an authorization misconfiguration in Global Permissions that caused the security risk. These are only two of the many kinds of security misconfiguration that can affect a business.', 'Types of Security Misconfiguration: ', '\u2192 AD misconfiguration: which exposes administrator and domain credentials.', '\u2192 Identity access misconfiguration: which provides attackers easy access to applications.', '\u2192 API security misconfiguration: which leaves unrestricted endpoints and unprotected files.', '\u2192 Network security misconfiguration: which is incorrect configuration of an information system.', '\u2192 Cloud security misconfiguration: which leaves gaps in the cloud environment that may lead to security breach.', '\u2192 Web server misconfiguration: which often includes unnecessary default and sample files.', 'Causes of Security Misconfiguration: ', '\u2192 Using default credentials or default passwords provided by a vendor', '\u2192 Installing unused features', '\u2192 Directory traversal', '\u2192 Accidental security-lax coding'],
    ...
    //   images: ['https://assets-global.website-files.com/5ff66329429d880392f6cba2/613b31e6b98d5547e3f10034_Insecure%20Deserialization%20work.png', 'https://www.fastly.com/cimages/6pk8mg3yh2ee/5dfTWonqkpg4oUU2PZCqed/950b5a88cfe0c4589d6abd54eecc9a14/image4.png?auto=avif'], // Add local image paths
    // },
    // {
    //   title: 'Vulnerable Components',
    //   content: ['A vulnerable and outdated component is a software component that is no longer being supported by the developer, making it susceptible to security vulnerabilities. Many times, a component has known vulnerabilities that don’t get fixed due to a lack of maintainer.', 'Applications often become vulnerable to attacks because they use outdated software components with known security vulnerabilities. Hackers can exploit these vulnerabilities to gain access to the application\'s data or to take control of the application entirely. Outdated software components are also more likely to contain security vulnerabilities, as timely patching is a vital part of security posture.',],
    ...
    //   images: [], // Add local image paths
    // },
    // {
    //   title: 'Missing Security Headers',
    //   content: ['Most modern browsers ships with a built in XSS filter. However this setting could be turned off by default. Including the X-XSS-Protection header forces this filter to be enabled, thus providing additional protection against Cross Site Scripting attacks.', 'Missing Strict Transport Security header means that the application fails to prevent users from connecting to it over unencrypted connections. An attacker able to modify a legitimate user’s network traffic could bypass the application’s use of SSL/TLS encryption, and use the application as a platform for attacks against its users. This attack is performed by rewriting HTTPS links as HTTP, so that if a targeted user follows a link to the site from an HTTP page, their browser never attempts to use an encrypted connection. The sslstrip tool automates this process.', '\u2022 For the steps on How to fix missing security headers, follow the given links'],
    ...
    //   images: ['https://media.geeksforgeeks.org/wp-content/uploads/20210611170329/7.PNG', 'https://namanadep.files.wordpress.com/2020/09/image.png'], // Add local image paths
    // },
